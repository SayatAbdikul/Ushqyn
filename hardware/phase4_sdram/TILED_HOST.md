# Phase 4 host-addressable tiled candidate
...
**boardless routed candidate**, not a physically validated release. The host
...
while the engine/DMA is busy receives BUSY status. STATUS reports combined
engine/DMA busy and error; its counters remain the engine's counters.
...
| `0x0e` | 1 | Reserved; use the ABORT frame to cancel active DMA |
...
per-node engine cycles and DMA payload bytes separately from host wall time, which
...
For boardless validation, `make p4-tiled-host` drives the actual packet
...
address, tile upload/run/readback, and exact
...
To route a fresh candidate, generate the target-specific encrypted HS IP as
...
the installed IDE library paths set. The [boardless route record](../../docs/research/evidence/phase4/boardless-tiled-host-route.json)
pins the source/IP/bitstream hashes and reports 22.008-MHz core Fmax at a
20.25-MHz generated clock, zero setup TNS, 12,151 logic, 18 BSRAM and 19.75
DSP equivalents. A connected board is still required for actual UART/SDRAM
model execution, memory bandwidth, latency, clock/reset reliability and power
measurements. The current SDRAM adapter remains single-outstanding, and the
planner does not yet create protected ping-pong tile regions or a streaming
burst schedule.

Gowin's [power estimate](../../docs/research/evidence/phase4/boardless-tiled-host-power.html.gz)
for this route is 162.510 mW total (122.800 mW quiescent, 39.710 mW dynamic)
...
The routed [candidate bitstream](../releases/phase4-sdram/hs_tiled_host_candidate.fs)
is archived for a future physical run. Its presence does not make it a tested
board release; check the route record's SHA-256 before programming it.
When the board is available, use the programming procedure in
...
Repeat with `rtl-vww` only after the KWS check passes. The port example is
the previously observed UART interface; discover the current device path
before use. Preserve the generated reports and the flashed bitstream hash.
