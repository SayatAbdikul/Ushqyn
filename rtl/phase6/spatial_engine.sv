...
module v2_engine #(
    ...
    logic signed [35:0] pw_acc[0:7],pw_next[0:7];
    ...
    always_comb begin
        ...
        for (integer k=0;k<8;k=k+1) begin
            products[k]=$signed(xword[k*8+:8])*$signed(spatial_pw?whex[col[2:0]*8+:8]:whex[k*8+:8]);
            pw_next[k]=pw_acc[k]+{{20{products[k][15]}},products[k]};
        ...
        case(state)
            ...
            PW_X_REQ:begin mem_req=1;address_full={40'b0,pw_input_addr[23:3],3'b0};end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            pw_input_addr<=0;pw_take<=0;pw_lane<=0;spatial_pw<=0;
        end else begin
            ...
            if(clear_counters)begin
                ...
                P_CHECK:begin
                    if(p0[63]||p0[63:32]==0||p1[7:0]>62||p1[63:40]!=0||
                        ...
                        if(spatial_pw)begin
                            for(integer k=0;k<8;k=k+1)pw_acc[k]<={{4{p0[31]}},p0[31:0]};
                            pw_take<=output_area-pixel_index>=8?4'd8:4'(output_area-pixel_index);
                            pw_lane<=0;pw_input_addr<=xb[23:0]+pixel_index[23:0];state<=PW_X_REQ;
                        end else state<=(op==OP_CONV||op==OP_DWCONV)?WIN_PREP:((op==OP_MAXPOOL||op==OP_AVGPOOL)?POOL_PREP:X_REQ);
                    end
                end
                X_REQ:if(mem_ready)state<=X_WAIT;
                X_WAIT:if(mem_rvalid)begin xword<=mem_rdata;if(op==OP_GEMM)state<=W_REQ;else state<=MAC;end
                PW_X_REQ:if(mem_ready)state<=PW_X_WAIT;
                PW_X_WAIT:if(mem_rvalid)begin
                    xword<=mem_rdata>>(32'(pw_input_addr[2:0])*8);
                ...
                PW_X2_WAIT:if(mem_rvalid)begin
                    xword<=xword|(mem_rdata<<((32'd8-32'(pw_input_addr[2:0]))*8));
                    state<=col[2:0]==0?PW_W_REQ:PW_MAC;
                end
                PW_W_REQ:begin
                    if(cached_weight)begin whex<=weight_cache[weight_cache_index];state<=PW_MAC;end
                    else if(mem_ready)state<=PW_W_WAIT;
                end
                PW_W_WAIT:if(mem_rvalid)begin
                    whex<=mem_rdata;weight_cache[weight_cache_index]<=mem_rdata;
                ...
                PW_MAC:begin
                    if((col[2:0]==7||col+1>=count)&&pw_overflow)fail(8'd5);
                        ...
                        else begin col<=col+1;pw_input_addr<=pw_input_addr+plane[23:0];state<=PW_X_REQ;end
                    end
                end
                WIN_PREP:begin
                    ...
                    if(row+1>=outputs)state<=NEXT_LAYER;
                        ...
                        if(spatial_pw)begin
                            if(pw_lane+1<pw_take)begin
                                ...
                                pixel_index<=pixel_index+1;state<=P_REQ;
                            end else begin
                                pixel_index<=0;oc<=oc+16'd1;output_channel_base<=output_channel_base+output_area;
                                weight_row<=weight_row+stride[23:0];weight_cache_valid<=0;state<=P_REQ;
                            end
